...
<asp:Content ID="Content1" ContentPlaceHolderID="Body_Content" Runat="Server">
    <asp:SqlDataSource runat="server" ID="sdsdata" 
        ...
        SelectCommand="select top 100 v_opr_service.service_id, offer_date,offer_no,broker_name,tax_sts,v_act_service.customer_name,v_act_service.an,v_act_service.customer_id, v_act_service.an_id,v_opr_service.branch_name, v_opr_service.marketing_id from v_opr_service inner join v_act_service on v_act_service.service_id=v_opr_service.service_id where service_status_id in ('1','2','6') and offer_no like @no and v_opr_service.service_id not in (select service_id from fin_proforma_service_opr) and v_opr_service.customer_id like case when @custid=0 then '%' else @custid end  and v_opr_service.an_id like case when @anid=0 then '%' else @anid end and v_opr_service.branch_id like @branch">
        <SelectParameters>
