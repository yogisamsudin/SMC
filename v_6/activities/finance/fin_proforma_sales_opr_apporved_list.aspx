...
<asp:Content ID="Content1" ContentPlaceHolderID="Body_Content" Runat="Server">
    <asp:SqlDataSource runat="server" ID="sdsdata" 
        ...
        SelectCommand="select top 100 v_opr_sales.sales_id, offer_date,offer_no,broker_name,tax_sts,v_act_sales.customer_name,v_act_sales.an,v_act_sales.customer_id, v_act_sales.an_id,v_opr_sales.branch_name, v_opr_sales.marketing_id from v_opr_sales inner join v_act_sales on v_act_sales.sales_id=v_opr_sales.sales_id where sales_status_id in ('1','2','6') and offer_no like @no and v_opr_sales.sales_id not in (select sales_id from fin_proforma_sales_opr) and v_opr_sales.customer_id like case when @custid=0 then '%' else @custid end  and v_opr_sales.an_id like case when @anid=0 then '%' else @anid end and v_opr_sales.branch_id like @branch">
        <SelectParameters>
