...
<asp:Content ID="Content1" ContentPlaceHolderID="Body_Content" Runat="Server">
    <asp:SqlDataSource runat="server" ID="sdsdata" 
        ...
        SelectCommand="select sales_status_marketing,sales_id, offer_no, dbo.f_convertDatetochar(offer_date)str_offer_date, offer_date,customer_name, grand_price,net,marketing_id, branch_name, reason_marketing + case when sales_status_id='5' and limit_approve_sts=0 then ' Nunggu Approval' else '' end reason_marketing from v_opr_sales where sales_status_id in ('2','5') and offer_no like @no and marketing_id like (select case when all_access=1 then '%' else marketing_id end from act_marketing where (user_id=@user or assistant_user_id=@user)) and customer_name like @cust and branch_id like @branchid">
        <SelectParameters>
